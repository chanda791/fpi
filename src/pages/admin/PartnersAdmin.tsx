...
const PartnersAdmin = () => {
  ...
  const load = async () => {
    try {
      setItems(await partnerService.getAll());
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      if (editingId) {
        await partnerService.update(editingId, form);
      } else {
  ...
  const remove = async (item: Partner) => {
    ...
    try {
      await partnerService.remove(item.id);
      await load();
    } catch (e) {
      console.error(e);
  ...
  return (
    <AdminLayout>
      <PageHeader
        title="Partners"
        subtitle="Manage the organisations shown on the public Partners page"
      />
      ...
      <PageCard>
        ...
        {loading ? (
          ...
          <EmptyState title="No Partners Yet" description="Add the first partner organisation." />
        ) : (
