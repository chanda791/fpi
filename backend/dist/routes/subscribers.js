...
router.post("/", rateLimit_1.publicWriteRateLimit, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.subscriber.create({ data: { email, name } });
...
router.delete("/:id", auth_1.requireAuth, async (req, res) => {
    try {
        await prisma_1.prisma.subscriber.delete({ where: { id: Number(req.params.id) } });
...
router.post("/broadcast", auth_1.requireAuth, async (req, res) => {
    try {
        const { subject, message } = req.body;
        if (!subject || !message) {
            return res.status(400).json({ message: "Subject and message are required" });
        }
        const subscribers = await prisma_1.prisma.subscriber.findMany({ where: { active: true } });
